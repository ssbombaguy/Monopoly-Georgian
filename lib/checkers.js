// Standard American checkers rules: mandatory captures, multi-jump, kinging.
// allowBackCapture is a lobby-chosen variant (Russian/Brazilian style): a man
// still can't move backward, but it can jump backward over an enemy piece.
export const SIZE = 8;
...
function movesForPiece(board, r, c, allowBackCapture) {
  ...
  const moveDirs = piece.king ? KING_DIRS : DIRS[piece.color];
  const captureDirs = piece.king || allowBackCapture ? KING_DIRS : DIRS[piece.color];
